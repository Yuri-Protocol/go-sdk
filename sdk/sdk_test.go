...
func TestComputeAccAddr(t *testing.T) {
	...
	tbaAddr, err := erc6551Sdk.Account(tokenId)
	assert.Nil(t, err)
	t.Logf("tbaAddr:%s", tbaAddr.String())
}
...
func TestTransferNativeToken(t *testing.T) {
	erc6551Sdk, err := NewSdk()
	assert.Nil(t, err)
	tbaAddr := common.HexToAddress("0x8DA2136FcE19e0FAd3C2364B2c696A2951Ef5d32")
	toAddr := common.HexToAddress("0xd1b9cef657ed8a2159171ddDa2085EFBf4e3A2aB")
	transfer, err := erc6551Sdk.Transfer(common.HexToAddress(utils.NullAddr), tbaAddr, toAddr, big.NewInt(700000))
	assert.Nil(t, err)
	t.Logf("txHash:%s", transfer.Hash().String())
}
...
func TestTransferErc20Token(t *testing.T) {
	...
	tbaAddr := common.HexToAddress("")
	toAddr := common.HexToAddress("0xd1b9cef657ed8a2159171ddDa2085EFBf4e3A2aB")
	transfer, err := erc6551Sdk.Transfer(ercTokenAddr, tbaAddr, toAddr, big.NewInt(60000000))
	assert.Nil(t, err)
	t.Logf("txHash:%s", transfer.Hash().String())
}
